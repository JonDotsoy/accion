...
namespace SourceTools {
    ...
    export const toSources = async function* (sources: StringLike[]): AsyncIterable<Source> {
        for (const source of sources) {
            ...
            if (exists) {
                ...
                if (stat.isDirectory()) {
                    yield* (await fs.readdir(location, { recursive: true })).map(e => ({ location: new URL(e, location) }))
                }
            }
        }
    }

    const uniqueSource = async function* (sources: AsyncIterable<Source>): AsyncIterable<Source> {
    ...
    const sortSource = async function* (sources: AsyncIterable<Source>): AsyncIterable<Source> {
        const mem = new Map((await Array.fromAsync(sources, e => [`${e.location}`, e])))
    ...
    const shasumSource = async function* (sources: AsyncIterable<Source>): AsyncIterable<Uint8Array> {
        for await (const source of sources) {
            ...
            yield new Uint8Array(await crypto.subtle.digest('SHA-256', await fs.readFile(url)))
        }
    }

    const sumShasum = async (shasumList: AsyncIterable<Uint8Array>): Promise<Uint8Array> => {
        let sum = new Uint8Array([])
        for await (const shasum of shasumList) {
            sum = new Uint8Array(await crypto.subtle.digest('SHA-256', new Uint8Array([
                ...sum,
                ...shasum,
            ])))
        }
    ...
    export const sourcesToShasum = (sources: StringLike[]) => {
        return sumShasum(shasumSource(sortSource(uniqueSource(toSources(sources)))))
    }
}
...
const readFileToStringSafe = async (target: StringLike): Promise<string | null> => {
    try {
        return await fs.readFile(new URL(target), 'utf-8')
    } catch (ex) {
        if (typeof ex === 'object' && ex !== null && Reflect.get(ex, 'code') === "ENOENT") return null
        throw ex
    }
}

const useNPMPack = async (): Promise<StringLike> => {
    const packsWorkspace = await useWorkspace('__npm_packs__', { cleanBefore: true })
    ...
    const shasumSource = await toHex(await SourceTools.sourcesToShasum([new URL("../src/", import.meta.url), new URL("../package.json", import.meta.url)]))

    if (shasumTarget === shasumSource) return packFile;

    const workspace = await useWorkspace(new URL("../", import.meta.url))

    const resultPack = await workspace.exec({ cmd: ["npm", "pack", "-y", "--json"] })
...
test("", async () => {
    ...
    const workspace = await useWorkspace(new URL(`.workspaces/pack/`, Bun.pathToFileURL(tmpdir())).toString(), {
        cleanBefore: true,
