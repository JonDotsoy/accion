...
export const exec = async (cmd: string, ...args: [args?: string[], options?: Options] | [options?: Options]) => {
    const log = debug("exec")
    let argOptions: Options = {}
    let cmdArgs: null | string[] = null

    for (const arg of args) {
        if (Array.isArray(arg)) { cmdArgs = arg; continue }
        if (typeof arg === 'object' && arg !== null) { argOptions = arg; continue }
    }
    ...
    const printStdoutAndStderr = verbose ? true : argOptions.silence ? false : true
    ...
    const matrixCmd = cmdArgs ? { cmd, cmdArgs } : { cmd: shell, cmdArgs: ['-c', cmd] }

    const subprocess = child_process.spawn(matrixCmd.cmd, matrixCmd.cmdArgs ?? [], {
        stdio: 'pipe',
        detached: false,
        shell: false,
    })
