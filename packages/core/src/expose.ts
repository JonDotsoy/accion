...
import { promiseWithResolvers } from "./common/promise-with-resolvers"
...
const incomingMessageToUint8Array = async (incomingMessage: http.IncomingMessage) => {
...
class Server {
    ...
    constructor(fetch: (request: Request) => Promise<Response>) {
        this.httpServer = http.createServer(async (req, res) => {
            try {
                const url = new URL(req.url ?? '', new URL(`http://${req.headers.host ?? 'localhost'}`, 'http://localhost'));
                ...
                const request = new Request(`${url}`, {
                    ...
                    method: req.method,
                    body: allowBody(req.method) ? await incomingMessageToUint8Array(req) : undefined,
                })
...
export class Expose {
    ...
    private async fetch(request: Request): Promise<Response> {
        ...
        return await handlers.router(url, request, this.manager) ?? new Response('', { status: 404 })
    }
    ...
    async listen(port: number = 6543, host: string = '127.0.0.1') {
        ...
        const server = this.server = new Server(req => {
            ...
            return this.fetch(req).then(r => (res = r, r)).finally(() => {
