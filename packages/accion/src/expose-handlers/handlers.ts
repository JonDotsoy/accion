...
export const flowchart = (manager: Manager) => {
  return new Response(manager.toFlowchart());
};
...
export const router = async (
  ...
  if (request.method === "GET" && url.pathname === "/status")
    return await status(manager);
  if (request.method === "GET" && url.pathname === "/flowchart")
    return await flowchart(manager);

  let matchURLCallingJob = /^\/job\/(?<jobId>.+)\/run$/.exec(url.pathname);
  if (request.method === "POST" && matchURLCallingJob !== null)
    return await callingJob(manager, matchURLCallingJob.groups!.jobId);
