...
test("render flowcharts and format the node id", () => {
  ...
  expect(
    manager.toFlowchart({ formatNodeId: (nodeId) => `__${nodeId}__` }),
  ).toMatchSnapshot();
});
...
test("render flowcharts and format the node label", async () => {
  ...
  expect(
    manager.toFlowchart({
      formatNodeLabel: (nodeId, job) =>
        `${nodeId}["${emojiState(manager.stateOfJob(job)?.state)} ${job.id.replace(/[\#\"]/g, (code) => `#${code.charCodeAt(0)};`)}"]`,
    }),
  ).toMatchSnapshot();
});
