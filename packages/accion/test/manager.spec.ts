...
import { State } from "../src/dto/state.ts";
...
test("sync multi manager", async () => {
  ...
  expect(manager1.manager.stateOfJob(manager1.job1)).toEqual(
    manager2.manager.stateOfJob(manager2.job1),
  );
});
...
test("calling job failed by dependencies", async () => {
  const job1 = job("1", () => 1);
  const job2 = job("2", { input: { job1 } }, ({ job1 }) => job1 + 2);
  const job3 = job("3", { input: { job1, job2 } }, ({ job1, job2 }) => ({
    job1,
    job2,
    job: 3,
  }));
...
test.skip("custom storage service", async () => {
  const job1 = job("1", () => 1);
  const job2 = job(
    "2",
    { input: { job1 } },
    ({ job1 }) => new Response(`[${job1}]`),
  );
  const job3 = job(
    "3",
    { input: { job1, job2 } },
    async ({ job1, job2 }) => `${job1} + ${await job2.clone().text()} + ${3}`,
  );
...
test.skip("subscribe DSN", async () => {
  ...
  await using server = new (class {
    ...
    constructor() {
      console.log(`Server ready on ${this.instanceServer.url}`);
    }

    get url() {
      return new URL("/event", this.instanceServer.url);
    }
  ...
  await using manager = Manager.create({
    dsn: `${server.url}`,
    jobs: [job1, job2, job3],
  });
