...
const createFormatterObject = (unitValue: typeof unit[number][0], ms: number) => {
...
export const durationFormat = (duration: number) => {
    ...
    const format = unitFound ? formats[unitFound[0]] : formats[unit.at(-1)?.[0] as any];
...
export const DurationFormat: FC<{
    ...
    return <time title={dateTime} dateTime={dateTime}>{durationFormat(value)}</time>
}
