...
export const LabelJob: FC<{ job: Job; enableReload?: boolean }> = ({
  job,
  ...props
}) => {
  ...
  return (
    <>
      <div className="grid grid-cols-[auto_1fr_auto] gap-1 items-center">
        <span>
          {isSuccess && (
            <CheckCircle className="aspect-square w-6 stroke-green-500" />
          )}
          {isFailed && <XCircle className="aspect-square w-6 stroke-red-500" />}
          {isPending && (
            <EllipsisHorizontalCircle className="aspect-square w-6 stroke-gray-300" />
          )}
          {isProgressing && (
            <ProgressCircle className="animate-spin aspect-square w-6 stroke-green-500" />
          )}
        </span>
        <span>{job.name}</span>
        <span>
